		...
		<form class="form-inline" role="form" action="" method="get">
			<div class="form-group">
				<input type="hidden" placeholder="提交号" class="form-control"
					name="rid"
					value="<?php if(isset($_GET['rid'])) echo (int)$_GET['rid']?>" />
			</div>
			<div class="form-group">
				 <input type="text" placeholder="题目编号" class="form-control"
					name="pid"
					value="<?php if(isset($_GET['pid']) && (int)$_GET['pid'] != 0)  echo (int)($_GET['pid']);?>" />
			</div>
			<div class="form-group">
				 <input type="text" placeholder="用户名" class="form-control"
					name="Programmer"
					value="<?php if(isset($_GET['Programmer'])) echo htmlspecialchars($_GET['Programmer'])?>" />
			</div>
			<div class="form-group">
		...
		<nav>
			<ul class="pagination pagination-lg text-center">
				<li><a
					...
					if ($contest)
						echo '&cid=' . $contest;
					echo '&pid=' . (int)get ( 'pid' );
					echo '&Programmer=' . htmlspecialchars(get ( 'Programmer' ));
					echo '&lang=' . (int)get ( 'lang' );
					echo '&status=' . (int)get ( 'status' );
					?>">首页</a></li>
				<li><a href="#">...</a></li>
				<li><a
					href="/status?end=
...
$(document).ready(function(){
		...
		if($contest)
			echo '&cid='.$contest;
		if(isset($_GET['start']))
			echo '&start='.(int)get('start');
		if(isset($_GET['end']))
			echo '&end='.(int)get('end');
		echo '&pid=' . (int)get ( 'pid' );
		echo '&Programmer=' . htmlspecialchars(get ( 'Programmer' ));
		echo '&lang=' . (int)get ( 'lang' );
		echo '&status=' . (int)get ( 'status' );
...
<?php if(isset($_SESSION['user_id']) &&  $_SESSION['privilege'][0] == 1) { ?>	
	function rejudge(id) {
