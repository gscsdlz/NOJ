				...
				<form class="navbar-form navbar-left" role="search" method="get"
					...
					<div class="form-group">
						<input type="text" class="form-control" placeholder="输入题目或者来源信息	"
							name="key"
							value="<?php if(isset($_GET['key'])) echo htmlspecialchars($_GET['key']);?>">
					</div>
...
<?php if($contest) {?>
		<marquee id="message" class="text-danger" onclick="window.location.href='/contest/asklist/<?php echo $contest;?>'"><h4>有关比赛的新通知，会在这里显示，点击即可查看！</h4></marquee>
<?php }?>
	</div>
...
<?php if($contest) {?>
	function get_message(){
		$.post("/contest/get_message", {cid:<?php echo $contest;?>}, function(data){
			var arr = eval("(" + data + ")");
