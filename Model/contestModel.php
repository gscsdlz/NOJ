...
class contestModel extends DB {
	...
	public function timeCheck($contestId) {
		$timeNow = time ();
		$times = parent::query ( "SELECT c_stime, c_etime FROM contest WHERE contest_id = ? ", array($contestId ));
		if ($times->rowCount () != 0) {
	...
	public function privilege_check($cid, $uid) {
		$res = parent::query_one ( "SELECT contest_pass FROM contest WHERE contest_id = ?", array($cid ));
		/**
		...
		if ($res) {
			$contest_pass = $res [0];
			if ($contest_pass != 1) {
				$res2 = parent::query_one ( "SELECT user_id FROM contest_user WHERE contest_id = ? AND user_id = ?",array($cid, $uid ));
				if (!$res2) {
	...
	public function check($user_id, $cid, $password) {
		...
		if($res) {
			if($password == $res[0]) {
				parent::query("INSERT INTO contest_user (user_id, contest_id) VALUES (?, ?)", array($user_id, $cid));
				return true;
			} else {
				return false;
			}
	...
	public function get_lists($cid = 0) {
		if ($cid) {
			$result = parent::query ( "SELECT contest.*, users.username FROM contest LEFT JOIN users ON (contest.user_id = users.user_id) WHERE contest_id = ? ORDER BY contest_id DESC", array($cid ));
		} else {
	...
	public function get_problem_list($contest_id, $needGets = 0) {
		$result = parent::query ( "SELECT inner_id, problem.pro_title, contest_pro.pro_id FROM contest_pro LEFT JOIN problem ON (problem.pro_id = contest_pro.pro_id) WHERE contest_id = ? ORDER BY inner_id", array($contest_id ));
		if ($result->rowCount () != 0) {
			// 获取用户每道题的提交记录
	...
	public function get_all_inner_id($contest_id) {
		...
		$res = parent::query("SELECT inner_id FROM contest_pro WHERE contest_id = ? ORDER BY inner_id", array($contest_id));
		if($res->rowCount() != 0) {
			while($row = $res->fetch(PDO::FETCH_NUM)) {
				$args[] = $row[0];
			}
			redisDB::setWithTimeOut('contestId:'.$contest_id, json_encode($args), REDISCACHETIME);
			return $args;
		} else {
			return null;
		}
	...
	public function get_real_Id($pid, $contest_id) {
		$result = parent::query_one( "SELECT pro_id FROM contest_pro WHERE contest_id = ? AND inner_id = ? LIMIT 1", array($contest_id, $pid ));
		if ($result) {
			return $result[0];
		} else {
			return - 1;
	...
	public function get_inner_Id($pid, $contest_id) {
		$result = parent::query_one( "SELECT inner_id FROM contest_pro WHERE contest_id = ? AND pro_id = ? LIMIT 1", array($contest_id, $pid ));
		if ($result) {
			return $result[0];
		} else {
			return - 1;
